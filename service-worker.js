...
self.addEventListener('fetch',e=>{
  ...
  if(isDoc){
    e.respondWith((async()=>{ try{ const net=await fetch(req,{cache:'no-store'}); return net; }catch(err){ const res=await caches.match(req); return res||caches.match('/'); } })());
  }else{
